...
class ProfileMenuBloc extends Bloc<ProfileMenuEvent, ProfileMenuState> {
  ProfileMenuBloc({required this.userRepository, required this.quoteRepository})
    : super(const ProfileMenuInProgress()) {
    on<ProfileMenuStarted>((_, emit) async {
      await emit.onEach(
        Rx.combineLatest2(
    ...
    on<ProfileMenuSignedOut>((_, emit) async {
      ...
      final newState = currentState.copyWith(isSignOutInProgress: true);
      emit(newState);
      await userRepository.signOut();
      await quoteRepository.clearCache();
    });

    on<ProfileMenuDarkPreferenceChanged>((event, _) async {
      await userRepository.upsertDarkModePreference(event.preference);
    });
