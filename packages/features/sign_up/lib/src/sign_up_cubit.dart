...
class SignUpCubit extends Cubit<SignUpState> {
  ...
  SignUpCubit({required this.userRepository}) : super(const SignUpState());
      ...
      username:
          shouldValidate
              ? Username.validated(
                newValue,
                isAlreadyRegistered:
                    newValue == previousUsername.value
  ...
  void onSubmit() async {
    ...
    if (!isFormValid) {
      try {
        ...
        final newState = state.copyWith(
          submissionStatus:
              error is! UsernameAlreadyTakeException &&
          ...
          email:
              error is EmailAlreadyRegisteredException
                  ? Email.validated(email.value, isAlreadyRegistered: true)
                  : state.email,
        );
        emit(newState);
      }
